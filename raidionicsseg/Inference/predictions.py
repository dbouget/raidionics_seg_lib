...
# import torch
# from raidionicsseg.Models.PLS.PLS import PLS
...
def run_predictions(data, model_path, parameters):
    ...
    if model_path.split('.')[-1] == 'ckpt':
        pass
        # return __run_predictions_pytorch(data, model_path, parameters)
    else:
...
def __run_predictions_pytorch(data, model_path, parameters):
    # net = torch.load(model_path, map_location=torch.device('cuda')) if torch.cuda.is_available() \
    #     else torch.load(model_path, map_location=torch.device('cpu'))

    # net = PLS.load_from_checkpoint(model_path)
    net = UnetModular.load_from_checkpoint(model_path)
    ...
    with torch.no_grad():
        ...
        probs = probs.cpu()                     # (c, x, y, z)
        mask = probs.cpu().numpy()

    final_result = np.moveaxis(mask, 0, -1)     # (c, x, y, z) -> (x, y, z, c)
...
def __run_predictions_whole(data, model, deep_supervision=False):
    ...
    if False:
        dump_feature_maps(model, data_prep)
